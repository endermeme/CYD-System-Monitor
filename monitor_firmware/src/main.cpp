...
void drawReactorScreen() {
  ...
  int infoY = startY + 4 * (cellH + gap) + 4;
  spr.setTextDatum(TL_DATUM);
  spr.setTextColor(COLOR_DIM, COLOR_BG);
  spr.drawString("CPU", startX, infoY, 1);
  spr.drawString("GPU", startX + 80, infoY, 1);

  infoY += 11;
  spr.setTextColor(COLOR_TEXT, COLOR_BG);
  spr.drawString(String((int)stats.cpu_temp) + "C", startX, infoY, 1);
  spr.drawString(String((int)stats.cpu_pwr) + "W", startX + 30, infoY, 1);
  spr.drawString(String(stats.gpu_temp) + "C", startX + 80, infoY, 1);
  spr.drawString(String((int)stats.gpu_pwr) + "W", startX + 110, infoY, 1);
